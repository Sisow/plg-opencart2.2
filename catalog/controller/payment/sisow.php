...
class ControllerPaymentSisow extends Controller {
	...
	public function _index($payment) {
		...
		if ($payment == 'sisowideal') {
			...
			if ($payment == 'sisowklarna') {
				...
				$data['text_monthly'] = $m > 0 ? $this->currency->format(round($m / 100.0, 2), $this->session->data['currency']) : false;
	...
	public function _redirectbank($payment) {
		...
		if ($payment == 'sisowklarna' || $payment == 'sisowklaacc' || $payment == 'sisowfocum' || $payment == 'sisowafterpay' || $payment == 'sisowcapayable' || $payment == 'sisowbillink') {
			if (!isset($this->request->post['sisowgender']) || $this->request->post['sisowgender'] == '') {
				$json['error'] = 'U heeft geen aanhef gekozen';
			}
			
			if($payment != 'sisowcapayable'){
		...
		{		
			...
			*/
			...
			foreach ($products_info as $key => $data) {
				...
				// product total
				$unit_price_incl_tax = $data['price'] + ($data['price'] * ($taxrate ? $taxrate / 100: 0 ) );
